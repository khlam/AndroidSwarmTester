...
func stmtFor() {
	...
	var vars []*Var
	switch choice("simple", "complex", "range") {//line92R
	case "simple":
		line("for %v {", rvalue(atype(ClassBoolean)))
	case "complex":
		line("for %v; %v; %v {", stmtSimple(true, nil), rvalue(atype(ClassBoolean)), stmtSimple(false, nil))
	case "range":
		switch choice("slice", "string", "channel", "map") {//line98R
		case "slice":
			t := atype(TraitAny)
			s := rvalue(sliceOf(t))
			switch choice("one", "two", "oneDecl", "twoDecl") {//line102R
			case "one":
		...
		case "string":
			s := rvalue(stringType)
			switch choice("one", "two", "oneDecl", "twoDecl") {//line121R
			case "one":
				line("for %v = range %v {", lvalueOrBlank(intType), s)
			case "two":
		...
		case "channel":
			cht := atype(ClassChan)
			ch := rvalue(cht)
			switch choice("one", "oneDecl") {//line141R
			case "one":
		...
		case "map":
			t := atype(ClassMap)
			m := rvalue(t)
			switch choice("one", "two", "oneDecl", "twoDecl") {//line154R
			case "one":
...
func stmtSimple(oas bool, newVars *[]*Var) string {
	...
	switch choice("empty", "inc", "assign", "oas", "send", "expr") {//line198R
	case "empty":
		return ""
	case "inc":
...
func stmtRecv() {
	...
	ch := rvalue(t)
	switch choice("normal", "decl") {//line239R
	case "normal":
...
func stmtSelect() {
	...
	for rnd(5) != 0 {
		...
		if rndBool() {
			...
			switch choice("one", "two", "oneDecl", "twoDecl") {//line306R
			case "one":
